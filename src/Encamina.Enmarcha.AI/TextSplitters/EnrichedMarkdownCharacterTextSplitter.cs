...
public partial class EnrichedMarkdownCharacterTextSplitter : EnrichedTextSplitter
{
    ...
    private static readonly Regex H4Regex = new(@"^#### .+", RegexOptions.Multiline | RegexOptions.Compiled);
    private static readonly Regex H5Regex = new(@"^##### .+", RegexOptions.Multiline | RegexOptions.Compiled);
    private static readonly Regex H6Regex = new(@"^###### .+", RegexOptions.Multiline | RegexOptions.Compiled);
    private static readonly Regex BdRegex = new(@"\*\*(.+?)\*\*", RegexOptions.Compiled);
    ...
    /// <inheritdoc/>
    public override IEnumerable<(IReadOnlyDictionary<string, string> Metadata, string Text)> SplitWithMetadata(string text, Func<string, int> lengthFunction, TextSplitterOptions options)
    {
    ...
    private static Dictionary<string, string> ExtractMetadata(string text)
    {
        ...
        {
            metadata["H1"] = header1Match.Groups[0].Value[2..].Trim();
        }
        ...
        })
        {
            ...
            if (matches.Count > 0)
            {
                ...
                metadata[$"H{level}"] = headers;
            }
        }

        // Extract bold text
        var boldMatches = BdRegex.Matches(text);
    ...
    private static Dictionary<string, string> UpdateContext(Dictionary<string, string> currentMetadata, Dictionary<string, string> previousContext)
    {
        ...
        var levels = new[] { "H1", "H2", "H3", "H4", "H5", "H6", "Bold" };
