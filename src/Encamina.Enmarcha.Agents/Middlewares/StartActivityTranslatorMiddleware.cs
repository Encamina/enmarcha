...
public class StartActivityTranslatorMiddleware : IMiddleware
{
    ...
    public async Task OnTurnAsync(ITurnContext turnContext, NextDelegate next, CancellationToken cancellationToken = default)
    {
        if (turnContext?.Activity?.IsStartActivity() == true)
        {
            ...
            {
                var messageActivities = activities.Where(a => a.Type == ActivityTypes.Message && !currentLanguage.MatchesWith(a.GetCultureInfoFromActivity())).Cast<Activity>().ToList();

                if (messageActivities.Count != 0)
                {
